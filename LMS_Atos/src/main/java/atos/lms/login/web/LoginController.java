...
public class LoginController {
	...
	/** LoginService: 로그인 서비스 객체 */
	@Resource(name = "atosLoginService")
	...
	/**
	 * 애플리케이션의 메인 페이지로 이동하는 처리 로직을 수행합니다.
	 *
	 * @param request HTTP 요청 객체
	 * @param model   뷰에 전달할 데이터를 담는 모델 객체
	...
	public String actionMain(HttpServletRequest request, ModelMap model) throws Exception {
		...
		LOGGER.debug("Globals.MAIN_PAGE > {}", main_page);

		if (main_page.startsWith("/")) { // 메인 페이지가 "/"로 시작하는 경우, 해당 페이지로 포워드
			return "forward:" + main_page;
		} else { // 설정된 메인 페이지가 있을 경우, 해당 페이지로 이동
			return main_page;
		}
	}
